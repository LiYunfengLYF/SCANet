...
def run(settings):
    ...
    # Create network
    if settings.script_name == 'scanet':
        ...
        if cfg.TRAIN.PRETRAIN.SOT_MODEL:
            ...
            if cfg.MODEL.HEAD.TYPE == cfg.MODEL.RGBS_HEAD.TYPE:
                net.sonar_head.load_state_dict(net.box_head.state_dict())
    ...
    # use_amp = getattr(cfg.TRAIN.TRAINER, "AMP", False)

    rgb_mode = getattr(cfg.DATA, 'RGB_MODE', False)
    print(f'LTR rgb mode: {rgb_mode}')

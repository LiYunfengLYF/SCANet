...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, LasHeR, SARDETSeq
...
def update_settings(settings, cfg):
    ...
    settings.rgbs_mode = cfg.DATA.RGBS_MODE
    settings.rgbs_rotate = cfg.DATA.RGBS_ROTATE


def names2datasets(name_list: list, settings, image_loader):
    assert isinstance(name_list, list)
    datasets = []
    print(fr' Dataset for RGBS mode is {settings.rgbs_mode} ')
    for name in name_list:
        ...
        if name == "LASOT":
            if settings.use_lmdb:
                ...
                datasets.append(Lasot(settings.env.lasot_dir, split='train', image_loader=image_loader,
                                      rgbs_mode=settings.rgbs_mode, rgbs_rotate=settings.rgbs_rotate))
        ...
        if name == "COCO17":
            if settings.use_lmdb:
                ...
                datasets.append(MSCOCOSeq(settings.env.coco_dir, version="2017", image_loader=image_loader,
                                          rgbs_rotate=settings.rgbs_rotate))
        if name == "VID":
            if settings.use_lmdb:
        ...
        if name == "TRACKINGNET":
            if settings.use_lmdb:
                ...
                datasets.append(
                    TrackingNet(settings.env.trackingnet_dir, image_loader=image_loader, rgbs_mode=settings.rgbs_mode,
                                rgbs_rotate=settings.rgbs_rotate))
...
def build_dataloaders(cfg, settings):
    ...
    print(f'sampler_mode: {sampler_mode},  RGB_MODE: {rgb_mode}')
    ...
    if train_cls:
        ...
        for n, p in net.named_parameters():
            if "cls" not in n:
                ...
                print(n)
    elif cfg.TRAIN.PRETRAIN.SOT_MODEL:
        if cfg.TRAIN.PRETRAIN.FREEZE_SOT_MODEL:
            for n, p in net.named_parameters():
                if "rgbs" not in n:
                    p.requires_grad = False
        param_dicts = [
            # {"params": [p for n, p in net.named_parameters() if "rgbs" in n and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "rgbs" not in n and p.requires_grad],
        ...
        if is_main_process():
            ...
            param_dicts = [
                ...
                {
                    "params": [p for n, p in net.named_parameters() if "sonar" in n and p.requires_grad],
                    "lr": cfg.TRAIN.LEARN.LR
                },
            ]
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "backbone" not in n and p.requires_grad]},
            {
                ...
                "lr": cfg.TRAIN.LEARN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            },
        ]
        if is_main_process():
